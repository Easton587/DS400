...
##
## Your answer:
## α =180, β = 120 (ratio 60:40, sum = 300).
Mean = 180/300 = 0.600
SD = √(αβ / (n²(n+1))) = √(180·120 / (300²·301)) ≈ 0.028
95% credible interval ≈ [0.545, 0.656]
...
##
## Your answer:
## Mean stays 0.450 in both cases (180/400 = 0.45, same as 45/100). 
## SD shrinks: from 0.050 down to about 0.025 (halved), so the 95% interval roughly halves in width too. 
## Why: the mean depends only on the ratio α:β, which is unchanged but the spread depends on α+β, the "effective sample size." Scaling both up means you're acting as if you'd seen 4× as much evidence for the same belief, so you're more confident (narrower interval) without shifting what you believe.
## ------------------------------------------------------------
...
##
## Your answer:
## Beta(1,1): Mean = 0.5, SD = √(1/12) ≈ 0.289, no mode.
## The curve is a perfectly flat horizontal line from 0 to 1. No mode because literally every value is equally plausible there's no peak to point to, not because the density is undefined, but because it never rises above any other point.
## Beta(0.5,0.5): Mean = 0.5, SD = √(0.125) ≈ 0.354, no mode.
## This one is U-shaped: density collapses toward zero in the middle and shoots up (toward infinity) at both edges, near 0 and near 1. No single mode here because there are two competing high-density regions at the extremes rather than one interior peak; the "most plausible" values are simultaneously near 0% and near 100%, which can't be reduced to a single mode.
## So: flat = no opinion anywhere; U-shaped = strong opinion, but split between two opposite extremes.
##
## ------------------------------------------------------------
...
##
## Your answer:
## The posterior curve lands exactly on top of the scaled likelihood curve.
## Why: Bayes' rule says posterior ∝ prior × likelihood. A flat (uniform) prior is a constant; it doesn't favor any value of π, so multiplying by it doesn't reshape the likelihood at all. With no real prior opinion, the data alone determines the posterior.
##
## ------------------------------------------------------------
...
##
## Your answer:
## n=50→500, y stays at 60%
## Original: prior (~100 pseudo-voters) = 67% weight, data (n=50) = 33% weight.
## At n=500, y=300: data weight becomes 500/(500+100) ≈ 83%, prior weight drops to ≈17%.
## Takeaway: to meaningfully overrule a moderately confident prior, your new sample needs to be several times larger than the prior's "effective sample size" (here, ~100). A poll of 50 barely dents a Beta(45,55) prior; a poll of 500 dominates it.
##
## ------------------------------------------------------------
...
##
## Your answer:
## The Bayesian 95% credible interval says: given this data and this prior, there's a 95% probability that Michelle's true support π actually falls in this range. 
## It's a direct probability statement about the parameter itself. The frequentist 95% confidence interval can't say that. It only says: if we repeated this exact sampling procedure many times, 95% of the intervals constructed this way would contain the true π. It's a statement about the long-run behavior of the procedure, not a probability statement about this one interval or this one π from the frequentist view, π is a fixed (if unknown) number, so it either is or isn't in this particular interval; there's no probability attached to that.
## It only says: if we repeated this exact sampling procedure many times, 95% of the intervals constructed this way would contain the true π. It's a statement about the long-run behavior of the procedure, not a probability statement about this one interval or this one π from the frequentist view, π is a fixed (if unknown) number, so it either is or isn't in this particular interval; there's no probability attached to that.
## ------------------------------------------------------------
...
##
## Your answer:
## Posterior = Beta(90+40, 10+160) = Beta(130, 170), mean ≈ 0.433.
## Distance from posterior to prior (0.9): 0.467
## Distance from posterior to data (0.2): 0.233
## The posterior lands roughly twice as close to the data as to the prior the data wins.
## This matches the weight sentence: prior effective size = 100, but now n = 200 (twice as large), so data weight = 200/300 ≈ 67%, prior weight ≈ 33% the reverse of Question 5.
## Why data wins here but not in Q5: it's purely about n relative to the prior's effective sample size (α+β=100). In Q5, n=50 < 100, so the prior dominated. Here, n=200 > 100, so the poll outweighs the prior regardless of how confident or extreme the prior was.
##
## ------------------------------------------------------------
...
##
## Your answer:
## Some ideas to draw on: the model implicitly assumes all "votes" the ones baked into the historical prior and the ones in the new poll are evidence about the same, unchanging π. 
## But if support is genuinely drifting (trending up, say, after a good debate), then older polls aren't just noisier versions of today's truth they're measuring a different true value. 
## Treating them as equally informative pseudo-counts means a strong prior built from months-old data can stubbornly resist a real, recent shift (as in Q5, where you needed a huge poll just to move the needle) the same mechanism that gives useful stability against noise also makes the model slow to detect real change. 
## If you suspected a genuine trend, you'd want the model to discount older data (e.g., weight recent polls more heavily, or shrink the effective size of the old prior over time) or replace the static-π assumption with something like a random walk / time-varying π, so recent data isn't fighting an outdated anchor on equal footing.
## ============================================================
